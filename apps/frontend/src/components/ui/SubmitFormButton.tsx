...
export const SubmitFormButton = ({ loading,type, children, className, disabled, ...props }: SubmitFormButtonProps) => {
  return (
    <button 
      ...
      className={`
        w-full relative overflow-hidden transition-all duration-300 bg-linear-to-r from-cyan-400 to-purple-600 
        text-black font-bold font-orbitron text-sm uppercase tracking-wider py-3.5 px-4 rounded-lg flex items-center justify-center gap-2 group
        ...
        hover:before:w-0 hover:before:h-0 cursor-pointer
        ${className}
      `}
    >
      {loading ? (
